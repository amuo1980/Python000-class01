...
def get_movie_comment_text(url):
    ...
    movie_comment_data = selector.xpath(
        "//span[@class='short']/text()")
    return ''.join(movie_comment_data)

# 分词

def cut_words(string):
    stop_words = './extra_dict/stop_words.txt'
...
def draw_wordcloud(text):
    ...
    plt.tight_layout()
    # 存储图像
    wc.to_file('./movie_bwbj.png')

if __name__ == '__main__':
    url = "https://movie.douban.com/subject/26752088/comments?status=P"
    movie_comment_text = get_movie_comment_text(url)
    word_cloud_text = cut_words(movie_comment_text)
